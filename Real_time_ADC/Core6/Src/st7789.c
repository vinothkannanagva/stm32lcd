   ...
   // ST7789_WriteCommand(ST7789_COLMOD);
	...
	/* Internal LCD Voltage generator settings */
	    ...
	    ST7789_WriteSmallData(0x20);			//	Default value
	  	...
	  	ST7789_WriteCommand(0xB0);                  //RAMCTRL (B0h): RAM Control
	  		{
	  			uint8_t data[] = {0b00000000, 0b11110101};
	  			ST7789_WriteData(data, sizeof(data));
	  		}
...
void ST7789_Fill_Color2()
{
	...
	ST7789_Select();
		...
		//	ST7789_WriteData(data, sizeof(data));
			ST7789_Select();
				ST7789_DC_Set();
			//	HAL_SPI_Transmit (&hspi1, hh, 1, 100);  // send the address from where you want to read data
				HAL_SPI_Transmit(&ST7789_SPI_PORT, 0xFF, 1, 100);
				HAL_SPI_Transmit(&ST7789_SPI_PORT, 0xFF, 1, 100);
				ST7789_UnSelect();
		}
...
void fillgraph2(float x ,float y,uint16_t width ,uint16_t height )
				{
	              ...
	              if(k>=170)
	              {
	            	  //clearing the data in empty portion
	            	    ST7789_DrawFilledRectangle(x, y,width, height,WHITE);//clear first data
	            	  // 160 is the upper data value by adjusting this value we can increase or decrease upper data position
				        ...
				        ST7789_DrawFilledRectangle(x, y,width, height,BLACK);//print first data
